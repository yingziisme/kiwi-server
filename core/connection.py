...
from core.utils import asr, vad, llm, tts, memory, intent, aec
...
class ConnectionHandler:
    def __init__(self, config: Dict[str, Any], mysql_utils, _vad, _asr, _llm, _tts, _memory, _intent):
        self.config = config
        ...
        self.output_list = queue.Queue(maxsize=50)
    ...
    def _create_processing_instances(self):
        memory_cls_name = self.agent["mem_model_id"] # 默认使用nomem
        ...
        asr_model_id = self.agent["asr_model_id"]
        vad_model_id = self.agent["vad_model_id"]
        llm_model_id = self.agent["llm_model_id"]
        tts_model_id = self.agent["tts_model_id"]
        intent_model_id = self.agent["intent_model_id"]
        aec_model_id = self.agent["aec_model_id"]
        ...
        return (
            vad.create_instance(
                vad_model_id,
                self.config["VAD"][vad_model_id]
            ),
            asr.create_instance(
                asr_model_id
                if not 'type' in self.config["ASR"][self.config["selected_module"]["ASR"]]
                else
                self.config["ASR"][asr_model_id]["type"],
                self.config["ASR"][asr_model_id],
                self.config["delete_audio"]
            ),
            llm.create_instance(
                llm_model_id
                if not 'type' in self.config["LLM"][llm_model_id]
                else
                self.config["LLM"][llm_model_id]['type'],
                self.config["LLM"][llm_model_id],
            ),
            tts.create_instance(
                tts_model_id
                if not 'type' in self.config["TTS"][tts_model_id]
                else
                self.config["TTS"][tts_model_id]["type"],
                self.config["TTS"][tts_model_id],
                self.config["delete_audio"]
            ),
            memory.create_instance(memory_cls_name, memory_cfg),
            intent.create_instance(
                intent_model_id
                if not 'type' in self.config["Intent"][intent_model_id]
                else
                self.config["Intent"][intent_model_id]["type"],
                self.config["Intent"][intent_model_id]
            ),
            aec.create_instance(
                aec_model_id
            ),
    ...
    async def handle_connection(self, ws):
        try:
            ...
            agent = self.mysql_utils.selectAgent(device_id)
            self.agent = agent[0]
            if agent:
                self.logger.bind(tag=TAG).info(f"查询结果: {agent[0]}")
                self._vad, self._asr, self._llm, self._tts, self._memory, self.intent, self.aec = self._create_processing_instances()
            else:
                self.logger.bind(tag=TAG).error(f"查询结果为空")
    ...
    def _initialize_components(self):
        ...
        if self.agent:
            self.prompt = self.agent["system_prompt"]
